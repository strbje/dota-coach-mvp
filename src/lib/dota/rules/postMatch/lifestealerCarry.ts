...
export function runLifestealerCarryPostMatchRules(match: NormalizedOpenDotaMatch): PostMatchAnalysis {
  ...
  if ((p?.laneReview?.deathsBefore10 ?? 0) > 0) laneFindings.push({ text: `${p?.laneReview?.deathsBefore10} смертей до 10:00 могли замедлить первый ключевой предмет.`, evidence: [], severity: 'warning' });
  ...
  if (!p || p.deathDataSource === 'unavailable' || !p.deathsByPhase) fightsFindings.push({ text: `В этом матче доступно только общее число смертей: ${deaths}. Разложить их по фазам OpenDota не позволяет.`, evidence: [], severity: 'info' });
  ...
  if (p?.economyByPhaseSource === 'gold_t/lh_t' && p.economyByPhase) {
    ...
    mapFindings.push({ text: `Лучший темп фарма был в ${getPhaseLabel(best[0]).toLowerCase()}: ${fmt(best[1].lhPerMinuteInPhase ?? 0)} LH/мин.`, evidence: [], severity: 'info' });
  ...
  return { matchId: match.matchId, hero: 'Lifestealer', role: 'carry', result, buildPlayed: p?.buildPlayed ?? [], timings: trackedTimings.reduce<Record<string, string>>((acc, t) => ((acc[t.item] = t.time), acc), {}), itemTimings: trackedTimings, economyByPhase: p?.economyByPhase, deathsByPhase: p?.deathsByPhase, farmProfile: p?.farmProfile,
