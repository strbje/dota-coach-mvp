...
export function GradesGrid({ grades }: { grades: PostMatchAnalysis['grades'] }) {
  return <section className="card"><h3>Оценки</h3><div className="grid grid-2">{Object.entries(grades).map(([key, grade]) => {
    ...
    return <article key={key} className={`card grade-card ${level.className}`}><div className="grade-head"><h4>{GRADE_LABELS[key] ?? key}</h4><span className={`badge ${level.className}`}>{level.label}</span></div><div className="grade-score">{grade.score}</div><div className="grade-bar"><span style={{ width: `${grade.score}%` }} /></div><p className="muted" style={{ marginBottom: '0.75rem' }}>{level.description}</p><ul>{grade.findings.slice(0, 3).map((f) => <li key={`${f.text}-${f.severity}`}>{f.text}{f.evidence.length ? <div style={{ opacity: 0.8 }}>Основание: {f.evidence.join(' · ')}</div> : null}</li>)}</ul></article>;
  })}</div></section>;
}
