...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    return NextResponse.json({ ok: true, source: 'opendota', matchId, hasApiKey, stages: { fetched: true, normalized: true }, normalized, goldReasonsDebug: { raw, ...goldReasonsDebug } });
  } catch (error) {
    const parsed = error instanceof Error ? error : new Error(String(error));
    const cause = parsed.cause as { message?: string } | undefined;
